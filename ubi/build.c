...
/* Slab cache for io entries */
struct kmem_cache *ubi_rc_entry_slab;
...
/*
 * Store the size necessary for the rc subsystem
 */
ubi->rc_size = ubi_calc_rc_size(ubi);
...
int ubi_detach_mtd_dev(int ubi_num, int anyway)
{
	...
	/* Update the rc subsystem PEBs with the read counters information */
	err = ubi_rc_write_at_detach (ubi);
	if (err < 0)
		return err;
#ifdef CONFIG_MTD_UBI_FASTMAP
...
static int __init ubi_init(void)
{
	...
	ubi_rc_entry_slab = kmem_cache_create("ubi_rc_entry_slab",
								sizeof(struct ubi_rc_entry),
								0, 0, NULL);
	if (!ubi_rc_entry_slab) {
		err = -ENOMEM;
		goto out_dev_unreg;
	}
...
out_slab:
	...
	kmem_cache_destroy(ubi_rc_entry_slab);
out_dev_unreg:
