...
retry:
	...
	/* Update the reading counter in the rc_lookuptbl*/
	if (ubi->is_attach){
		err2 = ubi_update_rc_attach(ubi,pnum);
		if (err2)
			return err2;
	}
	else{
		err2 = ubi_update_rc(ubi,pnum);
		if (err2)
			return err2;
	}
...
retry:
	...
	/* Reset the read counter of this pnum */
	if (ubi->is_attach){
		err = ubi_reset_rc_attach(ubi, pnum);
		if (err)
			return err;
	}
	else{
		err = ubi_reset_rc(ubi, pnum);
		if (err)
			return err;
	}
			 ...
			 struct ubi_vid_io_buf *vidb)
{
	...
	/* Update the vid_hdr->rc_sb */
	err = ubi_update_vid_rc_sb (ubi, pnum, vidb);
	if (err)
		return err;
