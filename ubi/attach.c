...
/**
 * init_attach_rc_lkt - initialize the rc_attachlkt
 ...
 */
 ...
 static int init_attach_rc_lkt (struct ubi_device *ubi)
 {
	 int err;
	 /* Error initialization */
	 err = -ENOMEM;
	 /* Allocate memory for the rc lookup table */
	 ubi->rc_attachlkt = kcalloc(ubi->peb_count, sizeof(void *), GFP_KERNEL);
	 if (!ubi->rc_attachlkt)
		 return err;
	 /* Initialize the rc_attachlkt to 0*/
	 for (int pnum = 0; pnum < ubi->peb_count; pnum++)
	 {
		 ubi->rc_attachlkt[pnum]->pnum = pnum;
		 ubi->rc_attachlkt[pnum]->rc = 0;
	 }
	 /* Open the is_attach semaphore*/
	 ubi->is_attach = 1;
	 return 0;
 }

 /**
 * ubi_update_rc_attach - Update the rc of a PEB defined by its pnum and
 * store it as an entry in the rc_attachlkt
 * @ubi: UBI device description object
 ...
 * case of failure.
 */
 int ubi_update_rc_attach (struct ubi_device *ubi, int pnum){
 int err;
 /* Error initialization */
 err = -ENOMEM;
 if (!ubi->rc_attachlkt)
 ...
 ubi->rc_attachlkt[pnum]->rc += 1;
 return 0;
}

/**
 * ubi_reset_rc_attach - Reset the rc of a PEB defined by its pnum and
 * store it as an entry in the rc_attachlkt
 * @ubi: UBI device description object
 * @pnum: Number of the PEB
 *
 ...
 * case of failure.
 */
int ubi_reset_rc_attach(struct ubi_device *ubi, int pnum){
 int err;
 /* Error initialization */
 err = -ENOMEM;
 if (!ubi->rc_attachlkt)
		    ...
		    int pnum, bool fast)
{
	...
	case UBI_IO_BAD_HDR:
      ...
      /* Check if this PEB with the VID Header corrupted is a rc superblock*/
      err = ubi_check_rc_sb_data (ubi, pnum);
      /* if it is mark it as superblock. It will be be erased after them
       * rc initialization because it is added to the erase list*/
      if (err == 0){
        ubi->rc_sb_pnum = pnum;
        ubi->rc_sb_pnum_counter +=1;
      }
	...
	/*
	* If this PEB is a rc superblock, store it in the ubi->rc_sb_pnum
	* and increment the counter
	*/
	if (be32_to_cpu(vidh->rc_sb) == UBI_VID_HDR_MAGIC)
	{
		ubi->rc_sb_pnum = pnum;
	  ubi->rc_sb_pnum_counter +=1;
	}
...
int ubi_attach(struct ubi_device *ubi, int force_scan)
{
	...
	/* Initialize the read counter in the attach */
	err = init_attach_rc_lkt(ubi);
	if (err<0)
		return err;
	/*
	* Initialize the ubi->rc_sb_pnum to UBI_UNKNOWN
	*/
	ubi->rc_sb_pnum = UBI_UNKNOWN;
	/*
	* Initialize the ubi->rc_sb_pnum_counter to 0
	*/
	ubi->rc_sb_pnum_counter = 0;
 ...
 /* Call the initialization of the rc subsystem before the wl but
 * after the volume table*/
	err = ubi_rc_init (ubi, ai);
	if (err)
		goto out_wl;
