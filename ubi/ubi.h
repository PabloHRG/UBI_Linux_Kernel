...
/**
 * struct ubi_rc_entry - entry to the PEBs rc_lookuptbl
 * @ rc: read counter
 * @ pnum: PEB number
 *
 * This data structure is used in the rc subsystem. Each PEB has a
 * &struct ubi_rc_entry object associated.
 */
 struct ubi_rc_entry{
	 int rc;
	 int pnum;
 ...
 * @rc_lookuptbl: a table to quickly find a &struct ubi_rc_entry object for any
 *								PEB
 * @rc_sb_pnum: PEB where the superblock of the rc is located
 * @rc_sb_pnum_counter: A counter to avoid any duplicity
 * @rc_attachlkt: a lookup table for storing the rc increasing during attaching,
 * 	while the rc subsystem is not yet initialized
 * @is_attach: a semaphore to know if update the rc_attachlkt
 *
...
struct ubi_device {
	...
	/* rc subsystem's stuff*/
	struct ubi_rc_entry **rc_lookuptbl;
	size_t rc_size;
	int rc_sb_pnum;
	int rc_sb_pnum_counter;
	struct ubi_rc_entry **rc_attachlkt;
	int is_attach;
...
extern struct kmem_cache *ubi_rc_entry_slab;
