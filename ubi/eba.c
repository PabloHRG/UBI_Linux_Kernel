		     ...
		     struct ubi_vid_io_buf *vidb)
{
  ...
  /********** rc subsystem checks ***********/
	...
	/* If the from PEB is the rc superblock, change the pnum for the Superblock
	 * both in RAM and in Flash (when the ubi_io_write_vid_hdr is called in this
   * function)
	 * The from PEB will be erased after the calling of ubi_eba_copy_leb in the
	 * function wear_leveling_worker. The only consumer of ubi_eba_copy_leb */
	if (from == ubi->rc_sb_pnum){
		ubi->rc_sb_pnum = to;
	  /* Read the original sb information and store it in rcsb for
		 * updating later */
		err = ubi_io_read_data(ubi, rcsb, from, 0, sizeof(*rcsb));
		if (err<0)
			return err;
		is_sb = 1;
	}
  if (!is_sb){
		/* Read the superblock information in order to know if the from PEB belongs
		 * to the rc PEBs but not the superblock*/
		err = ubi_io_read_data(ubi, rcsb, ubi->rc_sb_pnum, 0, sizeof(*rcsb));
		if (err<0)
			return err;
		/* Check if from PEB belongs to the rc PEBs */
		...
		/* if from PEB belongs to the rc PEBs -> update the rc_sb */
		if(belongs>0){
			err = ubi_update_rc_sb_eba(ubi, ubi->rc_sb_pnum, from, to);
			if (err<0)
				return err;
		}
	}
	/********** end rc subsystem checks ***********/
